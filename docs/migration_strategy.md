...
- **Shared tree:** Sets every biology discipline’s `GeographyTreeDefID` to match **Karplanter Moser** (skips Geologi). Then imports **`HIERARCHICAL_PLACE_OLD`** into `Geography` (Django ORM), adding a **Municipality** rank under **County** when missing.
