...
from flows.lib.specify_structure.config import DisciplineSpec, StructureConfig
...
def _create_division_row(institution: Institution, div_spec: DivisionSpec, result: ReconcileResult) -> Division | None:
...
def _ensure_collection(
    ...
    *,
    ...
    try:
        ...
        result.errors.append(f"collection {code!r} (discipline {discipline.name!r}): {e}")
        return
    result.collections_created += 1
    logger.info("Created collection %s for discipline %s", code, discipline.name)


def _dry_run_new_division_children(div_spec: DivisionSpec, result: ReconcileResult) -> None:
    """Count disciplines/collections when the division row does not exist yet (dry-run only)."""
